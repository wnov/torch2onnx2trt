...
__global__ void LinearKernel(const float *input, float *output,
                             ...
                             const int inChannel, const int outChanenel,
                             const int nElement) {
  int rowNb, colNb;
...
// class LinearPlugin
LinearPlugin::LinearPlugin(const std::string &name, const int inChannel,
                           const int outChannel, const float *weight,
                           const float *bias)
  ...
  memcpy(reinterpret_cast<void *>(const_cast<float *>(weight_)), weight,
         inChannel * outChannel * sizeof(float));
  memcpy(reinterpret_cast<void *>(const_cast<float *>(bias_)), bias,
         outChannel * sizeof(float));

  CHECK(cudaMalloc(&weight_d, inChannel * outChannel * sizeof(float)));
  CHECK(cudaMalloc(&bias_d, outChannel * sizeof(float)));
  CHECK(cudaMemcpy(reinterpret_cast<void *>(const_cast<float *>(weight_d)),
                   reinterpret_cast<const void *>(weight_),
                   inChannel * outChannel * sizeof(float),
                   cudaMemcpyKind::cudaMemcpyHostToDevice));
  CHECK(cudaMemcpy(reinterpret_cast<void *>(const_cast<float *>(bias_d)),
                   reinterpret_cast<const void *>(bias_),
                   outChannel * sizeof(float),
                   cudaMemcpyKind::cudaMemcpyHostToDevice));
}

LinearPlugin::LinearPlugin(const std::string &name, const void *buffer,
                           size_t length)
    : name_(name) {
  WHERE_AM_I();
  ...
  d += sizeof(int);
  weight_ = (float *)malloc(inChannel * outChannel * sizeof(float));
  memcpy(reinterpret_cast<void *>(const_cast<float *>(weight_)),
         const_cast<float *>(reinterpret_cast<const float *>(d)),
         inChannel * outChannel * sizeof(float));
  d += inChannel * outChannel * sizeof(float);
  bias_ = (float *)malloc(outChannel * sizeof(float));
  memcpy(reinterpret_cast<void *>(const_cast<float *>(bias_)),
         const_cast<float *>(reinterpret_cast<const float *>(d)),
         outChannel * sizeof(float));
...
LinearPlugin::~LinearPlugin() { WHERE_AM_I(); }
...
DataType LinearPlugin::getOutputDataType(int32_t index,
...
bool LinearPlugin::supportsFormatCombination(int32_t pos,
...
void LinearPlugin::configurePlugin(const DynamicPluginTensorDesc *in,
                                   int32_t nbInputs,
                                   const DynamicPluginTensorDesc *out,
                                   int32_t nbOutputs) noexcept {
  WHERE_AM_I();
  assert(nbInputs == 1 && nbOutputs == 1);
  // std::cout << int(in->desc.dims.nbDims) -1 << in->desc.dims.d[0] <<
  // in->desc.dims.d[1] << in->desc.dims.d[2] << in->desc.dims.d[3] <<
  // std::endl; std::cout << int(out->desc.dims.nbDims) -1 <<
  // out->desc.dims.d[0] << out->desc.dims.d[1] <<out->desc.dims.d[2]
  // <<out->desc.dims.d[3] <<std::endl;
  // assert(in->desc.dims.d[int(in->desc.dims.nbDims) -1] == inChannel);
  // assert(out->desc.dims.d[int(out->desc.dims.nbDims) - 1] == outChannel);
  return;
}

size_t LinearPlugin::getWorkspaceSize(const PluginTensorDesc *inputs,
                                      int32_t nbInputs,
...
int32_t LinearPlugin::enqueue(const PluginTensorDesc *inputDesc,
  ...
  cudaMalloc((void **)&transpoedWeight_d, sizeof(float) * nElement);
  {
    ...
    matrixTranspose<<<grid, block, 0, stream>>>(weight_d, transpoedWeight_d,
                                                height, width);
    CHECK(cudaDeviceSynchronize());
  }

  dim3 grid(CEIL_DIVIDE(nElement, 256), 1, 1), block(256, 1, 1);
  LinearKernel<<<grid, block, 0, stream>>>(
      (float *)inputs[0], (float *)outputs[0], transpoedWeight_d, bias_d,
      inChannel, outChannel, nElement);
...
void LinearPlugin::attachToContext(cudnnContext *contextCudnn,
...
LinearPluginCreator::LinearPluginCreator() {
  ...
  attr_.clear();
  attr_.emplace_back(
      PluginField("in_channel", nullptr, PluginFieldType::kINT32, 1));
  attr_.emplace_back(
      PluginField("out_channel", nullptr, PluginFieldType::kINT32, 1));
  attr_.emplace_back(
...
IPluginV2 *LinearPluginCreator::createPlugin(
  ...
  for (int i = 0; i < fc->nbFields; ++i) {
    ...
    if (filed_name == "in_channel") {
      ...
      weight = const_cast<float *>(
          reinterpret_cast<const float *>(fc->fields[i].data));
    } else if (filed_name == "bias") {
      bias = const_cast<float *>(
          reinterpret_cast<const float *>(fc->fields[i].data));
    }
  }
  LinearPlugin *pObj =
      new LinearPlugin(name, inChannel, outChannel, weight, bias);
...
void LinearPluginCreator::setPluginNamespace(
