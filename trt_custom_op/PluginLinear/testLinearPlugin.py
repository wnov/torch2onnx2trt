...
def getLinearPlugin(inChannel, outChannel, weight, bias):
    for c in trt.get_plugin_registry().plugin_creator_list:
        ...
        if c.name == pluginName:
            parameterList = []
            parameterList.append(trt.PluginField("in_channel", np.int32(inChannel), trt.PluginFieldType.INT32))
...
def run(shape, inchannel, outchannel, weight, bias):
    ...
    if os.path.isfile(trtFile):
        ...
        print("Start creating plugin!")
        linear_plugin = getLinearPlugin(inchannel, outchannel, weight, bias)
        pluginLayer = network.add_plugin_v2([inputT0], linear_plugin)
...
if __name__ == "__main__":
    ...
    weight = np.random.randn(32, 10).astype(np.float32)
    bias   = np.ones([10]).astype(np.float32)
    run([16, 32], 32, 10, weight, bias)
