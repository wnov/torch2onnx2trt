...
__global__ void
linearForwardKernel(const scalar_t *input, const scalar_t *weight,
                    const scalar_t *bias, scalar_t *out, size_t height,
                    size_t width, size_t inChannel) {
...
template <typename scalar_t>
__global__ void linearWeightBackwardKernel(const scalar_t *input,
                                           const scalar_t *gradOutput,
                                           scalar_t *gradWeight, size_t height,
...
template <typename scalar_t>
__global__ void linearBiasBackwardKernel(const scalar_t *gradOutput,
                                         scalar_t *gradBias, scalar_t width,
                                         size_t size) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
...
torch::Tensor linearCudaForward(torch::Tensor input, torch::Tensor weight,
  ...
  torch::Tensor output;
  if (input.dim() == 2)
    output = torch::zeros({outputDims[0], outputDims[1]},
                          torch::dtype(torch::kFloat32));
  else if (input.dim() == 3)
    output = torch::zeros({outputDims[0], outputDims[1], outputDims[2]},
                          torch::dtype(torch::kFloat32));
  else if (input.dim() == 4)
    output = torch::zeros(
  ...
  AT_DISPATCH_FLOATING_TYPES(weight.scalar_type(), "Linear_Cuda_Forward", [&] {
    linearForwardKernel<scalar_t><<<grid, block>>>(
        ...
        weight.transpose(0, 1).data_ptr<scalar_t>(), bias.data_ptr<scalar_t>(),
        output.view({-1, output.size(-1)}).data_ptr<scalar_t>(), size_t(width),
        size_t(height), size_t(weight.size(0)));
  });
  return output;
}
...
std::vector<torch::Tensor> linearCudaBackward(torch::Tensor gradOutput,
  ...
  dim3 block0(32, 32, 1);
  dim3 grid0((outChannel - 1) / block0.x + 1, (inChannel - 1) / block0.y + 1,
             1);
  AT_DISPATCH_FLOATING_TYPES(
      weight.scalar_type(), "LinearWeightCudaBackward", [&] {
        linearWeightBackwardKernel<scalar_t><<<grid0, block0>>>(
            ...
            gradWeight.data_ptr<scalar_t>(), size_t(inChannel),
            size_t(outChannel), size_t(size));
      });
  dim3 block1(1024, 1, 1);
  dim3 grid1((outChannel - 1) / block1.x + 1, 1, 1);
  AT_DISPATCH_FLOATING_TYPES(
      weight.scalar_type(), "LinearBiasCudaBackward", [&] {
        linearBiasBackwardKernel<scalar_t><<<grid1, block1>>>(
            gradOutput.data_ptr<scalar_t>(), gradBias.data_ptr<scalar_t>(),
            size_t(outChannel), size_t(size));
      });
  ...
  AT_DISPATCH_FLOATING_TYPES(
      weight.scalar_type(), "LinearInputCudaBackward", ([&] {
        linearWeightBackwardKernel<scalar_t><<<grid2, block2>>>(
            gradOutput.data_ptr<scalar_t>(), weight.data_ptr<scalar_t>(),
            gradInput.data_ptr<scalar_t>(), size_t(size), size_t(inChannel),
            size_t(outChannel));
      }));
